...
| `*.log`  `!important.log` | `debug.log`  *but not*  `logs/debug.log` | Prepending an exclamation mark to a pattern negates it. If a file matches a pattern, but *also* matches a negating pattern defined later in the file, it will not be ignored. |
| `/debug.log` | `debug.log`  *but not*  `logs/debug.log` | Patterns defined after a negating pattern will re-ignore any previously negated files. |
| `debug.log ` | `debug.log`  `logs/debug.log` | Prepending a slash matches files only in the repository root. |
